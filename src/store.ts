...
export const useStore = defineStore({
    ...
    actions: {
        async login(privateKey: string, accountIdStr: string): Promise<void> {
            if (__TEST__) {
                const { PrivateKey, AccountId } = await import(
                    "@hashgraph/sdk"
                );
        ...
        async networkPing(): Promise<boolean> {
            const { AccountBalanceQuery, AccountId } = await import(
                "@hashgraph/sdk"
            );
        ...
        async fetchNetworkNodeStakingInfo() {
            const container = useContainer();
            this.nodeStakingInfo =
                await container.cradle.hedera.getNodeStakingInfo(this.network);
        },

        async fetchAccountStakingInfo() {
            if (this.accountId == null || this.getClient == null) {
                throw new Error(
                    "tried to get account staking info without being logged in!"
                );
            }
            const container = useContainer();
            ...
            if (
                ...
                this.accountStakingInfo = {
                    staked_node_id: node === "" ? undefined : parseInt(node),
        ...
        async updateStake(declineRewards: boolean, node: number | null) {
            if (this.accountId == null || this.getClient == null) {
                throw new Error(
                    "tried to update stake without being logged in!"
                );
            }

            await this.getClient?.updateStaking({
                node,
                declineRewards,
        ...
        async errorMessage(error: Error): Promise<string> {
            ...
            const { StatusCodes, TransportStatusError } = await import(
            ...
            if (error instanceof StatusError) {
                switch (error.status) {
                    ...
                    case Status.PayerAccountNotFound:
                        return i18n.global
                            .t("Common.Error.PayerNotReal")
                            .toString();
                    case Status.PayerAccountUnauthorized:
                        return i18n.global
                            .t("Common.Error.PayerFrozen")
                            .toString();
                    case Status.ReceiptNotFound:
                        return i18n.global
                            .t("Common.Error.NoReceipt")
                            .toString();
                    case Status.RecordNotFound:
                        return i18n.global
                            .t("Common.Error.NoRecord")
                            .toString();
                    case Status.ResultSizeLimitExceeded:
                        return i18n.global
                            .t("Common.Error.BigResults")
                            .toString();
                    ...
                    case Status.TokenNotAssociatedToFeeCollector:
                        return i18n.global
                            .t("Common.Error.TokenNoFeeCollector")
                            .toString();
                    ...
                    case Status.TokenHasNoSupplyKey:
                        return i18n.global
                            .t("Common.Error.TokenConfigBroken")
                            .toString();
                    case Status.TokenWasDeleted:
                        return i18n.global
                            .t("Common.Error.TokenDeleted")
                            .toString();
                    case Status.TokensPerAccountLimitExceeded:
                        return i18n.global
                            .t("Common.Error.TooManyTokens")
                            .toString();
                }
                ...
                switch ((error as TransportStatusError).statusCode) {
                    case StatusCodes.CONDITIONS_OF_USE_NOT_SATISFIED:
                        return i18n.global
                            .t("Common.Error.LedgerDenyRequest")
                            .toString();
                    default:
                        return i18n.global
                            .t("Common.Error.LedgerError")
                            .toString();
                }
            }
